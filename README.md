...
第一次打开会走一遍引导：**欢迎 → 激活 → 权限 → 试一下**。

「激活」可以点「以后再说」——邀请码可能还在别人手里。它会直接关掉窗口，不会把人送进一个
注定失败的试说。「权限」和「试一下」只有凭据、麦克风和辅助功能都就绪才能往下走。

**关掉窗口就不会再自动弹。** 无论是点「完成」、点「以后再说」还是直接点红色关闭键，引导都不会在
下次启动时重新出现——凭据、麦克风、辅助功能三样齐了算完成，没齐则算「用户自己关掉的」。这两条都
必须成立：只认「完成」按钮的话，走完全程却用红点关窗的人会在此后每次启动都被这个窗口抢走焦点；
而一台被 MDM 禁掉辅助功能的 Mac 根本走不完，会被永远困住。想再看一遍，菜单栏里有「使用指南…」。

「试一下」那一步就在引导窗口里说，文字出现在窗口的框里，不会打进别的 App——Whisper 自己在最
前面的时候本来就没有可写入的目标，平时按热键会直接提示「请先切回需要输入文字的 App」，这一步
把那个时刻改成了排练：热键、麦克风、连接、整理全都照常跑，只是不注入。切到别的 App 再说话仍然
是正常写入，行为没变。

新安装会在**凭据没配好或者没给辅助功能权限**时显示引导；已经在用的机器升级上来不会看到它。一旦真实引导已经开始，
未完成的 setup 会优先于这个升级兼容判断，直到麦克风也授权、或者用户自己把窗口关掉为止。除了菜单栏的
「使用指南…」，也可以：
...
想看**新用户第一次打开是什么样**（自己这台机器早就配好了，看不到激活和授权那两屏）：

```bash
defaults write com.mingyili.Whisper showOnboarding preview
```

preview 把界面按「什么都没配」渲染，并且**什么都不碰**：点「允许」「打开设置」「激活」都只是
模拟一下让流程能走完（打勾动画照常），不动系统权限、不动钥匙串、不注册登录项。试说会显示并锁定当前真实触发键，
因为排练仍由正在运行的真实热键监听驱动；它不会改 UserDefaults 或重启监听。左下角会显示
「预览」以免和真的搞混。两个键都是读一次就自动清掉，下次启动显示一次。
